...
#Sparse trellis
  ...
  Nsamples=5
  for minLeaves in 12 #sbatch --array 0 submitHPC_HCmanagerSparseTrellis.s
...
#  for minLeaves in 36 #sbatch --array 0-49 submitHPC_HCmanager.s
  ...
  do
    ...
    singularity exec --nv \
          ...
          bash -c "source /ext3/env.sh; python $SCRATCH/HCmanager/src/HCmanager/run_evaluate.py --dataset_dir=$SCRATCH/ginkgo/data/invMassGinkgo/SparseTrellis/ --dataset=jets_${minLeaves}N_${Nsamples}trees_${tcut}tcut_${SLURM_ARRAY_TASK_ID}.pkl --NleavesMin=$minLeaves --output_dir=$SCRATCH/HCmanager/experiments/ginkgo/SparseTrellis/ --results_filename=outjets_${minLeaves}N_${Nsamples}trees_${tcut}tcut_${SLURM_ARRAY_TASK_ID}.pkl --wandb_dir=$SCRATCH/HCmanager --max_leaves=20 --all_pairs_max_size=${all_pairs_max_size} --algorithm=${algorithm} --exact_heuristic_proof=False --approx_heuristic=${approx_heuristic} --exact_astar_approx_heuristic=${exact_astar_approx_heuristic} --num_tries_m=${num_tries_m} --num_tries_k=${num_tries_k} --maxBeamJets=1000000 --max_nodes=${max_nodes} "
